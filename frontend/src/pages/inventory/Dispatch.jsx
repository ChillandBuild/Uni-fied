...
import { Plus, Eye, Save, X, Truck, CheckCircle, Activity, Package } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { useSortableTable, SortableHeader } from '../../hooks/useSortableTable';
import { api } from '../../lib/api';

const emptyForm = () => ({
  dispatch_id: `DSP-${Math.floor(Math.random() * 10000)}`, date: new Date().toISOString().split('T')[0],
  customer_name: '', destination: '', vehicle_no: '', driver_name: '', status: 'Pending', items: []
});
...
export default function Dispatch() {
  ...
  const fetchRecords = async () => {
    try { setRecords(await api.get('/inventory/dispatch') || []); } catch {}
  };
  ...
  const handleCreate = async () => {
    setLoading(true);
    try {
      await api.post('/inventory/dispatch', form);
      await fetchRecords(); showMsg('Dispatch created!'); setMode('list');
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-[#111827]">{ro ? 'Dispatch Details' : 'New Dispatch'}</h2>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#e5e7eb] p-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-xs font-medium text-[#6b7280] mb-1">Dispatch ID</label><input name="dispatch_id" value={form.dispatch_id || ''} readOnly className={inp(true)} /></div>
            ...
            <div><label className="block text-xs font-medium text-[#6b7280] mb-1">Destination</label><input name="destination" value={form.destination || ''} onChange={handleField} readOnly={ro} className={inp(ro)} /></div>
            <div><label className="block text-xs font-medium text-[#6b7280] mb-1">Vehicle No</label><input name="vehicle_no" value={form.vehicle_no || ''} onChange={handleField} readOnly={ro} className={inp(ro)} /></div>
            <div><label className="block text-xs font-medium text-[#6b7280] mb-1">Driver Name</label><input name="driver_name" value={form.driver_name || ''} onChange={handleField} readOnly={ro} className={inp(ro)} /></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => { setForm(emptyForm()); setMode('new'); }} className="flex items-center gap-2 px-5 py-2.5 bg-[#0891b2] text-white rounded-lg text-sm font-medium hover:bg-[#0e7490] shadow-sm"><Plus size={16} />New Dispatch</button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-[#e5e7eb] bg-[#f9fafb]">
            <tr>
              ...
              <SortableHeader label="Vehicle" sortKey="vehicle_no" sortConfig={sortConfig} onSort={requestSort} />
          ...
          <tbody className="divide-y divide-[#e5e7eb]">
            {sorted.map(t => (
              <tr key={t.dispatch_id} className="hover:bg-[#f9fafb]">
                <td className="px-5 py-3.5 font-medium text-[#0891b2]">{t.dispatch_id}</td>
                <td className="px-5 py-3.5 text-[#374151]">{t.date}</td>
                <td className="px-5 py-3.5 text-[#374151]">{t.customer_name}</td>
                <td className="px-5 py-3.5 text-[#374151]">{t.vehicle_no}</td>
                <td className="px-5 py-3.5"><span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#e0e7ff] text-[#3730a3]">{t.status}</span></td>
            ...
            {sorted.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-[#6b7280]">No dispatch records found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
